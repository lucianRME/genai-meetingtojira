...
def test_run_agentic(sample_vtt, db_conn, stub_chat, monkeypatch):
    ...
    sid = rp.ensure_session(db_conn, "primark", "unit-sid")
    res = rp.run_agentic(sample_vtt, "primark", sid, db_conn)
...
def test_run_classic(sample_vtt, db_conn, stub_chat):
    sid = rp.ensure_session(db_conn, "primark", "unit-sid-2")
    res = rp.run_classic(sample_vtt, "primark", sid, db_conn)
